...
export default function LandingMidnight() {
  ...
  return (
    <main className="bg-[#030308]">
      {/* HERO */}
      <section className="relative min-h-screen flex items-center pt-20 pb-24 overflow-hidden">
        {/* Midnight gradient orbs */}
        <div className="absolute top-[-10%] left-1/2 -translate-x-1/2 w-[900px] h-[600px] bg-[radial-gradient(ellipse,rgba(99,102,241,0.07)_0%,transparent_70%)]" />
        <div className="absolute bottom-20 right-[10%] w-[400px] h-[400px] bg-[radial-gradient(circle,rgba(234,179,8,0.04)_0%,transparent_70%)]" />
        <div className="absolute top-40 left-[5%] w-[300px] h-[300px] bg-[radial-gradient(circle,rgba(139,92,246,0.05)_0%,transparent_70%)]" />
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="flex justify-center mb-10">
              <div className="inline-flex items-center gap-2.5 px-6 py-3 rounded-full bg-indigo-500/[0.08] border border-indigo-500/20 backdrop-blur-sm">
                ...
                <span className="text-sm text-indigo-300 font-bold tracking-wider">MIDNIGHT VPN</span>
            ...
            <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="text-lg sm:text-xl text-indigo-200/70 max-w-2xl mx-auto mb-10 leading-relaxed">
              หรูหรา ลึกลับ ปลอดภัย ดั่งแสงดาวยามค่ำคืน
            </motion.p>
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }} className="flex flex-col sm:flex-row items-center justify-center gap-4">
              ...
              <Link href="/login" className="px-8 py-4 rounded-2xl border border-indigo-500/30 text-indigo-300 font-bold text-lg hover:bg-indigo-500/10 transition-all">
                เข้าสู่ระบบ
              </Link>
            </motion.div>
          </motion.div>
      ...
      <section className="py-24 relative">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16">
            <span className="text-indigo-400 text-sm font-bold tracking-wider">FEATURES</span>
            <h2 className="text-3xl sm:text-4xl font-black text-white mt-3">หรูหรายามค่ำคืน</h2>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {[
              { icon: Zap, title: 'ความเร็ว 10Gbps', desc: 'เร็วดั่งแสงดาว', color: 'indigo' },
              { icon: Shield, title: 'เข้ารหัส AES-256', desc: 'ปลอดภัยระดับสูงสุด', color: 'purple' },
              { icon: Globe, title: '50+ ประเทศ', desc: 'ครอบคลุมทุกมุมโลก', color: 'amber' },
              { icon: Clock, title: 'Uptime 99.9%', desc: 'เสถียรตลอดกลางคืน', color: 'violet' },
              { icon: Users, title: 'รองรับหลายอุปกรณ์', desc: 'ใช้งานพร้อมกันได้', color: 'blue' },
              { icon: Headphones, title: 'ซัพพอร์ต 24/7', desc: 'พร้อมช่วยเหลือทุกเวลา', color: 'fuchsia' },
            ].map((f, i) => (
              <motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }}
                className="group p-6 rounded-2xl bg-indigo-500/[0.03] border border-indigo-500/10 hover:border-indigo-500/30 hover:bg-indigo-500/[0.06] transition-all">
                ...
                <p className="text-sm text-indigo-200/50">{f.desc}</p>
              </motion.div>
            ))}
          </div>
        </div>
