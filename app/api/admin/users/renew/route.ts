...
// Minimal VPN Panel API for updateClient
class VpnPanelAPI {
  private host: string
  private port: number
  private path: string
  ...
  // Get inbound details to find the current client settings
  ...
  // Update client on the 3x-ui panel with new expiry time
  async updateClient(inboundId: number, clientUUID: string, newExpiryTime: number, remark: string, subId: string): Promise<{ success: boolean; error?: string }> {
    ...
    // First get current inbound to find the client's current settings
    ...
    // Build client data - use existing settings if found, otherwise construct from known values
    const clientData = {
      id: clientUUID,
    ...
    // 3x-ui updateClient endpoint: POST /panel/api/inbounds/updateClient/{uuid}
...
// GET - Fetch VPN orders for a specific user
export async function GET(request: NextRequest) {
  try {
    ...
    // Get user info (for discount check)
    ...
    // Get all VPN orders for this user
    ...
    return NextResponse.json({ error: 'Failed to fetch VPN orders' }, { status: 500 })
  }
}

// POST - Renew a VPN code (extend expiry, deduct balance, update panel)
export async function POST(request: NextRequest) {
  try {
    ...
    if (!orderId || !days || days < 1 || days > 30) {
      return NextResponse.json({ error: 'กรุณาระบุ orderId และจำนวนวัน (1-30)' }, { status: 400 })
    }

    // Get the VPN order with server details
    ...
    // Calculate price (same as buy flow)
    ...
    if (order.user.balance < totalPrice) {
      return NextResponse.json({
        error: `เครดิตของผู้ใช้ไม่เพียงพอ (มี: ${order.user.balance} ฿, ต้องการ: ${totalPrice} ฿)`
      }, { status: 400 })
    }
    ...
    if (!updateResult.success) {
      ...
      return NextResponse.json({
        error: `ไม่สามารถอัปเดตบนเซิร์ฟเวอร์ VPN ได้: ${updateResult.error}`
      }, { status: 500 })
    }

    // Update database in transaction: update order expiry + deduct user balance
    ...
    return NextResponse.json({
      success: true,
      message: `ต่ออายุสำเร็จ ${days} วัน (หักเงิน ${totalPrice} ฿)`,
      newExpiry: newExpiryTime.toISOString(),
      deducted: totalPrice
    })
  } catch (error) {
    console.error('Failed to renew VPN:', error)
    return NextResponse.json({ error: 'Failed to renew VPN' }, { status: 500 })
  }
}
