...
// ─── Form Capture Script (intercept all forms and POST data to API) ───
...
" onclick="if(window.__adLink){window.open(window.__adLink,'_blank')}">
  <img src="${imgSrc}" alt="Ad" style="max-width:90vw;max-height:65vh;border-radius:16px;object-fit:contain;box-shadow:0 25px 50px -12px rgba(0,0,0,0.5);pointer-events:none;" />
  <div style="
...
// ─── Password Gate (full page, dark themed) ───
function generatePasswordGateHtml(pageTitle: string) {
  return `<!DOCTYPE html>
<html lang="th">
<head>
  <meta charset="UTF-8">
  ...
  <div class="container">
    ...
    if (page.password) {
      if (!pwParam || pwParam !== page.password) {
        // If pw param wrong, return 403 for AJAX / return gate page for direct visit
        if (pwParam) {
          return new Response('Unauthorized', { status: 403 })
        }
        return new Response(generatePasswordGateHtml(page.title), {
          status: 200,
    ...
    // Form capture script (always inject — will only activate if page has <form>)
    ...
    // Ad overlay — determine which ad to show (page-specific or global)
