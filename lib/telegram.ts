...
export async function sendDiscordNotification(message: string) {
  ...
  try {
    ...
    if (!config || !config.discordEnabled || !config.discordWebhookUrl) {
      console.log('[Discord] Skipped: config missing or disabled')
      return
    }
    ...
    console.log('[Discord] Sending to:', url.hostname)
        ...
        (res) => {
          ...
          res.on('end', () => {
            try {
              ...
              console.log('[Discord] Response:', json.id ? 'ok' : 'failed')
              resolve({ ok: !!json.id, data: json })
            } catch {
              console.error('[Discord] Invalid JSON response:', data)
    ...
    if (!result.ok) {
      console.error('[Discord] Send failed:', result.data)
    }
  } catch (error) {
...
export async function notifyTopup(userName: string, amount: number, method: string, balance: number) {
  ...
  await sendDiscordNotification(
    msg.replace(/<\/?(b|code|pre)>/g, '**').replace(/<\/?b>/g, '**').replace(/<\/?code>/g, '`').replace(/<\/?pre>/g, '```')
  )
}
...
export async function notifyRegister(userName: string, email: string) {
  ...
  await sendTelegramNotification(msg)
  await sendDiscordNotification(
    msg.replace(/<\/?b>/g, '**').replace(/<\/?code>/g, '`').replace(/<\/?pre>/g, '```')
  )
}
